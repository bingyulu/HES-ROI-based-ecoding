...
# when using GPU, set memory growth to true to avoid CUDNN_STATUS_INTERNAL_ERROR
...
class TransNetV2:
    ...
    def predict_video(self, video_fn: str):
        ...
        ### !!! ffmpeg read in rgb24, cv2 is in BGR
        video = []
        if os.path.isdir(video_fn ):
            for img in sorted(glob.glob(os.path.join(video_fn, '*'))):
                frame = cv2.resize(cv2.cvtColor(cv2.imread(img), cv2.COLOR_BGR2RGB), (48, 27))
                video.append(frame)
        else:
            ...
            while True:
                ...
                if not ret:
                    break
                # cv2 default channel is BGR,convert to RGB, then resize
                frame = cv2.resize(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), (48, 27))
                video.append(frame)
